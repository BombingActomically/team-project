...
$db_registrations = $pdo->query("
    SELECT r.registration_id, r.registration_type, r.status, r.registered_at,
           ...
           COALESCE(s.gender, 'female') AS gender,
  ...
  <body>
    ...
    <div class="pc-container">
      <div class="pc-content container-fluid">
          ...
          <div class="col-12 col-sm-6 col-xl-3">
            <div class="card stat-card p-3">
              <div class="d-flex align-items-center justify-content-between">
                ...
                <div class="stat-icon icon-info"><i class="bi bi-calendar-event"></i></div>
              </div>
          ...
          <div class="col-12 col-sm-6 col-xl-3">
            <div class="card stat-card p-3">
              <div class="d-flex align-items-center justify-content-between">
                ...
                <div class="stat-icon icon-purple"><i class="bi bi-ticket-perforated"></i></div>
              </div>
        ...
        <div class="row g-3 mb-4">
          <div class="col-12 col-xl-7">
            <div class="main-card h-100">
              ...
              <div class="p-4">
                <div class="chart-box" style="height: 280px;"><canvas id="lineChart"></canvas></div>
              </div>
            </div>
          </div>
          ...
          <div class="col-12 col-xl-5">
            <div class="main-card h-100">
              ...
              <div class="p-4 d-flex flex-column align-items-center justify-content-center">
                <div class="chart-box" style="height: 210px; width: 210px;"><canvas id="donutChart"></canvas></div>
              ...
              <div class="p-4">
                <div class="row g-3 align-items-end">
                  ...
                  <div class="col-12 col-md-1 d-flex gap-1">
                    <button type="button" class="btn btn-primary w-100 p-2" onclick="applyFilters()" title="Apply Filter"><i class="bi bi-funnel"></i></button>
                    <button type="button" class="btn btn-outline-secondary w-100 p-2" onclick="resetFilters()" title="Reset Filters"><i class="bi bi-arrow-clockwise"></i></button>
                  </div>
                </div>
              </div>
            </div>
        ...
        <!-- [ Data Tables with Limit & Pagination ] start -->
        <div class="row g-3 mb-4">
          ...
          <div class="col-12 col-xl-7">
            <div class="main-card">
              ...
              <!-- Footer Limit & Pagination for Registrations -->
              <div class="card-footer bg-white border-top py-3">
                <div class="row align-items-center g-3">
                  <div class="col-md-6 col-12">
                    <div class="d-flex align-items-center gap-2">
                      <small class="text-muted text-nowrap">Show</small>
                      <select class="form-select form-select-sm w-auto" id="regLimitSelect" style="font-size: 12px; padding-top: 2px; padding-bottom: 2px;" onchange="changeRegLimit(this.value)">
                      ...
                      <small class="text-muted ms-2" id="regShowingCountText">Showing 0 of 0 entries</small>
                    </div>
                  </div>
                  <div class="col-md-6 col-12">
                    <nav aria-label="Registrations pagination">
                      <ul class="pagination pagination-sm mb-0 justify-content-md-end justify-content-center" id="regPagination"></ul>
                    </nav>
                  </div>
                </div>
              </div>
          ...
          <div class="col-12 col-xl-5">
            <div class="main-card">
              ...
              <!-- Footer Limit & Pagination for Payments -->
              <div class="card-footer bg-white border-top py-3">
                <div class="row align-items-center g-3">
                  <div class="col-md-6 col-12">
                    <div class="d-flex align-items-center gap-2">
                      <small class="text-muted text-nowrap">Show</small>
                      <select class="form-select form-select-sm w-auto" id="payLimitSelect" style="font-size: 12px; padding-top: 2px; padding-bottom: 2px;" onchange="changePayLimit(this.value)">
                        <option value="5">5</option>
                        <option value="10" selected>10</option>
                        <option value="25">25</option>
                        <option value="50">50</option>
                      ...
                      <small class="text-muted text-nowrap me-2">entries</small>
                      <span class="text-muted opacity-50">|</span>
                      <small class="text-muted ms-2" id="payShowingCountText">Showing 0 of 0 entries</small>
                    </div>
                  </div>
                  <div class="col-md-6 col-12">
                    <nav aria-label="Payments pagination">
                      <ul class="pagination pagination-sm mb-0 justify-content-md-end justify-content-center" id="payPagination"></ul>
                    </nav>
                  </div>
                </div>
              </div>
    ...
    <!-- Dynamic DB Frontend Script with Limit & Pagination -->
    <script>
      var masterDataset = {
        baseKpis: {
      ...
      var filteredRegsCache = masterDataset.registrations;
      var filteredPaymentsCache = masterDataset.payments;
      ...
      function applyFilters() {
        ...
        filteredRegsCache = masterDataset.registrations.filter(function (r) {
        ...
        filteredPaymentsCache = masterDataset.payments.filter(function (p) {
        ...
        var mult = filteredRegsCache.length > 0 ? (filteredRegsCache.length / Math.max(1, masterDataset.registrations.length)) : 0.2;
        ...
        animateValue('kpi-regs-val', 0, filteredRegsCache.length, 350, false);
        animateValue('kpi-rev-val', 0, Math.round(base.revenue * (isFiltered ? mult : 1)), 350, true);

        if (chartLine) {
          chartLine.data.datasets[0].data = [1, 2, 3, 5, 6, 7, filteredRegsCache.length];
          chartLine.update('active');
        }
      }

      function changeRegLimit(val) {
        regPageSize = parseInt(val, 10);
        regCurrentPage = 1;
        renderRegTable();
      }

      function changePayLimit(val) {
        payPageSize = parseInt(val, 10);
        payCurrentPage = 1;
        renderPayTable();
      }
      ...
      function renderRegTable() {
        ...
        var totalMatched = filteredRegsCache.length;
        document.getElementById('regCountBadge').innerText = totalMatched + " Records";

        var totalPages = Math.ceil(totalMatched / regPageSize) || 1;
        if (regCurrentPage > totalPages) regCurrentPage = totalPages;
        if (regCurrentPage < 1) regCurrentPage = 1;

        var startIdx = (regCurrentPage - 1) * regPageSize;
        var endIdx = startIdx + regPageSize;
        var pageSlice = filteredRegsCache.slice(startIdx, endIdx);

        regBody.innerHTML = pageSlice.length ? '' : '<tr><td colspan="4" class="text-center text-muted py-4">No matching records</td></tr>';
        ...
        pageSlice.forEach(function (r) {
          ...
          var avatarSrc = '';
          if (r.avatar && r.avatar !== 'avatar.png') {
            avatarSrc = r.avatar;
          } else {
            avatarSrc = (r.gender && r.gender.toLowerCase() === 'male') ? 'assets/images/user/avatar-2.jpg' : 'assets/images/user/avatar-1.jpg';
          }
        ...
        var visibleCount = Math.min(regPageSize, totalMatched - startIdx > 0 ? totalMatched - startIdx : 0);
        document.getElementById('regShowingCountText').textContent = `Showing ${visibleCount} of ${totalMatched} entries`;
        renderRegPagination(totalPages);
      }

      function renderPayTable() {
        var payBody = document.getElementById('payTableBody');
        var totalMatched = filteredPaymentsCache.length;
        document.getElementById('payCountBadge').innerText = totalMatched + " Records";

        var totalPages = Math.ceil(totalMatched / payPageSize) || 1;
        if (payCurrentPage > totalPages) payCurrentPage = totalPages;
        if (payCurrentPage < 1) payCurrentPage = 1;

        var startIdx = (payCurrentPage - 1) * payPageSize;
        var endIdx = startIdx + payPageSize;
        var pageSlice = filteredPaymentsCache.slice(startIdx, endIdx);

        payBody.innerHTML = pageSlice.length ? '' : '<tr><td colspan="4" class="text-center text-muted py-4">No transaction records found</td></tr>';
        ...
        var visibleCount = Math.min(payPageSize, totalMatched - startIdx > 0 ? totalMatched - startIdx : 0);
        document.getElementById('payShowingCountText').textContent = `Showing ${visibleCount} of ${totalMatched} entries`;
      ...
      function buildPageList(current, total) {
        var pages = [];
        var windowSize = 1;
        var start = Math.max(2, current - windowSize);
        var end = Math.min(total - 1, current + windowSize);
      ...
      function renderGenericPagination(containerId, currentPage, totalPages, onGoTo) {
        ...
        function makeItem(label, opts) {
          ...
          a.style.cursor = opts.disabled ? "default" : "pointer";
          a.innerHTML = label;
          if (!opts.disabled && !opts.active && typeof opts.page === 'number') {
            a.addEventListener("click", function () { onGoTo(opts.page); });
          }
          li.appendChild(a);
          return li;
        }
        ...
        buildPageList(currentPage, totalPages).forEach(function (p) {
          if (p === '...') {
            ...
            container.appendChild(makeItem(String(p), { active: p === currentPage, page: p }));
          }
        });
      ...
      function renderRegPagination(totalPages) {
        renderGenericPagination('regPagination', regCurrentPage, totalPages, function (page) {
          regCurrentPage = page;
          renderRegTable();
        });
      }

      function renderPayPagination(totalPages) {
        renderGenericPagination('payPagination', payCurrentPage, totalPages, function (page) {
          payCurrentPage = page;
          renderPayTable();
        });
      }
      ...
      function sortTable(tableType, colIdx) {
        ...
        var list = isReg ? filteredRegsCache : filteredPaymentsCache;
      ...
      function showRegDetails(regId) {
        ...
        if (!reg) return;
        var avatarSrc = '';
        if (reg.avatar && reg.avatar !== 'avatar.png') {
          avatarSrc = reg.avatar;
        } else {
          avatarSrc = (reg.gender && reg.gender.toLowerCase() === 'male') ? 'assets/images/user/avatar-2.jpg' : 'assets/images/user/avatar-1.jpg';
        }
