                                ...
                                <?php else: ?>
                                    <?php foreach ($members as $i => $m): ?>
                                        <?php 
                                            ...
                                            // Gender-aware avatar resolution
                                            $photoFile = trim($m['profile_photo'] ?? '');
                                            $gender = strtolower(trim($m['gender'] ?? ''));
                                            $fullServerPath = $PHOTO_DIR . $photoFile;

                                            if (!empty($photoFile) && $photoFile !== 'avatar.png' && file_exists($fullServerPath)) {
                                                $photoSrc = $PHOTO_WEB_PATH . htmlspecialchars($photoFile);
                                            } else {
                                                if ($gender === 'male') {
                                                    $photoSrc = 'assets/images/user/avatar-2.jpg';
                                                } else {
                                                    $photoSrc = 'assets/images/user/avatar-1.jpg';
                                                }
                                            }
                                        ?>
                                        <tr>
