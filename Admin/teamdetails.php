...
<head>
    <title>Team Details | Eventura Admin</title>
                ...
                <div class="card main-card shadow-sm mb-4">
                    ...
                    <div class="card-body p-4">
                        <div class="d-flex align-items-center gap-4 flex-wrap">
                            <?php 
                                $leaderPhotoFile = trim($team['leader_photo'] ?? '');
                                $leaderGender = strtolower(trim($team['leader_gender'] ?? ''));
                                $leaderServerPath = $PHOTO_DIR . $leaderPhotoFile;

                                if (!empty($leaderPhotoFile) && $leaderPhotoFile !== 'avatar.png' && file_exists($leaderServerPath)) {
                                    $leaderPhoto = $PHOTO_WEB_PATH . htmlspecialchars($leaderPhotoFile);
                                } else {
                                    $leaderPhoto = ($leaderGender === 'male') ? 'assets/images/user/avatar-2.jpg' : 'assets/images/user/avatar-1.jpg';
                                }
                            ?>
                                    ...
                                    <?php else: ?>
                                        <?php foreach ($members as $index => $m): ?>
                                            <?php 
                                                ...
                                                // Gender-aware resolver for members
                                                $memberPhotoFile = trim($m['profile_photo'] ?? '');
                                                $memberGender = strtolower(trim($m['gender'] ?? ''));
                                                $memberServerPath = $PHOTO_DIR . $memberPhotoFile;

                                                if (!empty($memberPhotoFile) && $memberPhotoFile !== 'avatar.png' && file_exists($memberServerPath)) {
                                                    $photo = $PHOTO_WEB_PATH . htmlspecialchars($memberPhotoFile);
                                                } else {
                                                    $photo = ($memberGender === 'male') ? 'assets/images/user/avatar-2.jpg' : 'assets/images/user/avatar-1.jpg';
                                                }
                                            ?>
                                            <tr>
                                                <td><?= $index + 1 ?></td>
